...
_DIMENSIONS = [
    ...
    ("读者友好度", "本章出现的专有术语/新设定/角色/关系是否对读者（尤其是中途加入的读者）有最小必要解释——\n                 判断标准：①一个新术语首次出现时是否用了 5-15 字做内嵌解释（'塔纹——他掌心那道自幼就有的印记'）\n                 ②跨章引用的旧设定是否有一句轻量提示（'锈脉——上次断魂崖那根贯穿他血脉的暗管'）\n                 ③读者合上书能否在 10 秒内复述本章发生了什么\n                 这是防止世界观过度膨胀导致读者脱轨的核心维度，权重与伏笔回收并列"),
    ("追读欲", "章末是否让读者想立刻点开下一章——这是唯一面向'好看'而非'自洽'的维度：\n                 判断标准：①本章主角的主动行动是否带来了可见的局势变化（爽点兑现，不是'感到一阵震撼'）\n                 ②冲突是否比章首升级了一级\n                 ③章末钩子是否具体可感（'他要查的账，查到了自己名下'），而非空悬套话（'一个巨大的秘密即将揭开'）\n                 ④全章是否以静止对话倒设定为主（是则直接 ≤4 分）\n                 低分必须给出'删掉哪段、把哪段改成行动'的具体改法"),
